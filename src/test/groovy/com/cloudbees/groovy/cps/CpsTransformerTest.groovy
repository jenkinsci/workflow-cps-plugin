...
''')==5;
    ...
    // TODO: Move the CpsDefaultGroovyMethods stuff to a separate paremeterized test.

    private static final String eachString = """
    def x = 100;
    (0..10).each { y -> x+=y; }
    return x;
"""
...
"""
    ...
    @Test
    void syncEach() {
        assert evalCPS(codeAsMethod(eachString)) == 155;
    }

    private static final String eachMapKVString = """
    def x = 100
    def m = [a: 1, b: 2, c: 3];
    m.each { k, v -> x += v }
    return x;
"""
    ...
    @Test
    void eachMapKV() {
        assert evalCPS(eachMapKVString) == 106;
    }

    @Test
    void syncEachMapKV() {
        assert evalCPS(codeAsMethod(eachMapKVString)) == 106;
    }

    private static final String eachMapEntryString = """
    def x = 100
    def m = [a: 1, b: 2, c: 3];
...
"""
    ...
    @Test
    void eachMapEntry() {
        assert evalCPS(eachMapEntryString) == 106;
    }

    @Test
    void syncEachMapEntry() {
        assert evalCPS(codeAsMethod(eachMapEntryString)) == 106;
    }

    private static final String setEachString = """
    def x = 100
...
"""
    ...
    @Test
    void setEach() {
        assert evalCPS(setEachString) == 106
    }

    @Test
    void syncSetEach() {
        assert evalCPS(codeAsMethod(setEachString)) == 106;
    }

    private final static String sortedSetEachString = """
    def x = 100
...
"""
    ...
    @Test
    void sortedSetEach() {
        assert evalCPS(sortedSetEachString) == 106
    }

    @Test
    void syncSortedSetEach() {
        assert evalCPS(codeAsMethod(sortedSetEachString)) == 106;
    }

    private final static String collectListString = """
    return [1, 2, 3].collect { it * 2 }
"""

    @Test
    void collectList() {
        assert evalCPS(collectListString) == [2, 4, 6]
    }

    @Test
    void syncCollectList() {
        assert evalCPS(codeAsMethod(collectListString)) == [2, 4, 6];
    }

    private final static String collectListIntoExistingListString = """
    def existing = [2]
    return [2, 3, 4].collect(existing) { it * 2 }
"""

    @Test
    void collectListIntoExistingList() {
        assert evalCPS(collectListIntoExistingListString) == [2, 4, 6, 8]
    }

    @Test
    void syncCollectListIntoExistingList() {
        assert evalCPS(codeAsMethod(collectListIntoExistingListString)) == [2, 4, 6, 8];
    }

    private static final String collectListIntoExistingSetString = """
    return [2, 3, 4].collect([2] as HashSet) { it * 2 }
"""

    @Test
    void collectListIntoExistingSet() {
        assert evalCPS(collectListIntoExistingSetString) == [2, 4, 6, 8] as HashSet
    }

    @Test
    void syncCollectListIntoExistingSet() {
        assert evalCPS(codeAsMethod(collectListIntoExistingSetString)) == [2, 4, 6, 8] as HashSet;
    }

    private static final String collectSetString = """
    return ([1, 2, 3] as HashSet).collect { it * 2 }
"""

    @Test
    void collectSet() {
        assert evalCPS(collectSetString) == [2, 4, 6]
    }

    @Test
    void syncCollectSet() {
        assert evalCPS(codeAsMethod(collectSetString)) == [2, 4, 6]
    }

    private static final String collectSetIntoExistingListString = """
    def existing = [2]
    return ([2, 3, 4] as HashSet).collect(existing) { it * 2 }
"""

    @Test
    void collectSetIntoExistingList() {
        assert evalCPS(collectSetIntoExistingListString) == [2, 4, 6, 8]
    }

    @Test
    void syncCollectSetIntoExistingList() {
        assert evalCPS(codeAsMethod(collectSetIntoExistingListString)) == [2, 4, 6, 8]
    }

    private static final String collectSetIntoExistingSetString = """
    return ([2, 3, 4] as HashSet).collect([2] as HashSet) { it * 2 }
"""

    @Test
    void collectSetIntoExistingSet() {
        assert evalCPS(collectSetIntoExistingSetString) == [2, 4, 6, 8] as HashSet
    }

    @Test
    void syncCollectSetIntoExistingSet() {
        assert evalCPS(codeAsMethod(collectSetIntoExistingSetString)) == [2, 4, 6, 8] as HashSet
    }

    private static final String collectMapKVString = """
    def m = [a: 1, b: 2, c: 3];
    return m.collect { k, v -> v }
"""

    @Test
    void collectMapKV() {
        assert evalCPS(collectMapKVString) == [1, 2, 3];
    }

    @Test
    void syncCollectMapKV() {
        assert evalCPS(codeAsMethod(collectMapKVString)) == [1, 2, 3]
    }

    private static final String collectMapEntryString = """
    def m = [a: 1, b: 2, c: 3];
    return m.collect { e -> e.getValue() }
"""

    @Test
    void collectMapEntry() {
        assert evalCPS(collectMapEntryString) == [1, 2, 3];
    }

    @Test
    void syncCollectMapEntry() {
        assert evalCPS(codeAsMethod(collectMapEntryString)) == [1, 2, 3]
    }

    private static final String collectMapKVIntoExistingListString = """
    def m = [a: 2, b: 3, c: 4];
    return m.collect([1]) { k, v -> v }
"""

    @Test
    void collectMapKVIntoExistingList() {
        assert evalCPS(collectMapKVIntoExistingListString) == [1, 2, 3, 4];
    }

    @Test
    void syncCollectMapKVIntoExistingList() {
        assert evalCPS(codeAsMethod(collectMapKVIntoExistingListString)) == [1, 2, 3, 4]
    }

    private static final String collectMapEntryIntoExistingListString = """
    def m = [a: 2, b: 3, c: 4];
    return m.collect([1]) { e -> e.getValue() }
"""

    @Test
    void collectMapEntryIntoExistingList() {
        assert evalCPS(collectMapEntryIntoExistingListString) == [1, 2, 3, 4];
    }

    @Test
    void syncCollectMapEntryIntoExistingList() {
        assert evalCPS(codeAsMethod(collectMapEntryIntoExistingListString)) == [1, 2, 3, 4]
    }

    private static final String collectMapKVIntoExistingSetString = """
    def m = [a: 2, b: 3, c: 4];
    return m.collect([1] as HashSet) { k, v -> v }
"""

    @Test
    void collectMapKVIntoExistingSet() {
        assert evalCPS(collectMapKVIntoExistingSetString) == [1, 2, 3, 4] as HashSet;
    }

    @Test
    void syncCollectMapKVIntoExistingSet() {
        assert evalCPS(codeAsMethod(collectMapKVIntoExistingSetString)) == [1, 2, 3, 4] as HashSet
    }

    private static final String collectMapEntryIntoExistingSetString = """
    def m = [a: 2, b: 3, c: 4];
    return m.collect([1] as HashSet) { e -> e.getValue() }
"""

    @Test
    void collectMapEntryIntoExistingSet() {
        assert evalCPS(collectMapEntryIntoExistingSetString) == [1, 2, 3, 4] as HashSet;
    }

    @Test
    void syncCollectMapEntryIntoExistingSet() {
